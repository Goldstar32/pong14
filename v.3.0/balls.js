...
export class Ball {
    ...
    // Update speeds
    updateSpeed() {
        this.applyDrag(); // Apply drag forces

        if (this.yAcc !== 0) {
            if (Math.abs(this.yAcc) < 0.05) {
                this.yAcc = 0;
            } else {
                this.yAcc *= 0.98;  // Smaller decay factor for smoother spin
            }
            this.ySpeed += this.yAcc;  // Update speed based on acceleration
        }

        this.checkMinSpeed(); // Check minimum speed requierment
    }
